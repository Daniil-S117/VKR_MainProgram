...
class KeyPointVal:
    ...
    def validate_set(self, path, dataset):
        ...
        for index, data in enumerate(dataset):
            ...
            print("вывод сделан")
            # возьмите его как массив numpy и уменьшите размерность на единицу
            heatmaps = heatmaps.detach().numpy().squeeze(0)

            # постройте тепловые карты в папке запуска
            ...
            # создайте тепловые карты в папке запуска
            ...
            print("извлеченные ключевые моменты")
            ...
            # выделенные ключевые моменты на графике
            key_point_file_path = os.path.join(
                path, KEYPOINT_DIR, KEY_POINT_PREFIX + image_name + '.jpg')
            # также измените размер исходного изображения
        ...
        # Оцените общие показатели и сохраните их в файл
        full_metrics_dict = {}
...
def plot_heatmaps(heatmaps1, heatmaps2, filename):
    ...
    if N_HEATMAPS == 1:
        ...
        heatmap_plot = plt.imshow(heatmaps2.squeeze(0),
                                  ...
                                  vmax=1)
        plt.colorbar(heatmap_plot, shrink=0.5)
        plt.title('Истинная тепловая карта')
        ...
        for i in range(3):
            im = axs[0, i].imshow(heatmaps1[i],
                                  ...
                                  vmin=0,
                                  vmax=1)
            axs[0, i].set_title(f'Прогнозируемая тепловая карта {titles[i]}')
        im1 = im
        for i in range(3):
            im = axs[1, i].imshow(heatmaps2[i],
                                  ...
                                  vmin=0,
                                  vmax=1)
            axs[1, i].set_title(f'Истинная тепловая карта {titles[i]}')
        im2 = im
...
def plot_key_points(image, key_points_pred, key_points_true, filename):
    ...
    if N_HEATMAPS == 1:
        ...
        plt.scatter(key_points[:, 0],
                    ...
                    c='red',
                    marker='x')
        plt.title('Предсказанная ключевая точка')
        ...
        plt.scatter(key_points[:, 0],
                    ...
                    c='red',
                    marker='x')
        plt.title('Истинная ключевая точка')
    else:
        titles = ['Начало", "Середина", "Конец']
        for i in range(3):
            ...
            plt.scatter(key_points[:, 0],
                        ...
                        c='red',
                        marker='x')
            plt.title(f'Предсказанная ключевая точка {titles[i]}')
        ...
        for i in range(3):
            ...
            plt.scatter(key_points[:, 0],
                        ...
                        c='red',
                        marker='x')
            plt.title(f'Истинная ключевая точка {titles[i]}')

    # Измените расположение подзаголовков
    plt.tight_layout()
